...
class TripService:
    ...
    Изолирует внешний слой (контроллеры, API) от деталей CQRS:
    маршрутизирует вызовы к нужным Command/Query-обработчикам.
    """
    ...
    def __init__(
        ...
        # Query handlers
        self._get_by_id_handler = GetTripByIdHandler(repository)
    ...
    def create_trip(
        ...
        """Создаёт новую поездку. Возвращает trip_id."""
    ...
    def add_place_to_route(self, trip_id: str, place_id: str) -> None:
        """Добавляет место в маршрут поездки."""
    ...
    def add_note(self, trip_id: str, text: str) -> str:
        """Добавляет заметку к поездке. Возвращает note_id."""
    ...
    def get_trip_by_id(self, trip_id: str) -> Optional[TripDto]:
        """Возвращает TripDto по идентификатору или None."""
        query = GetTripByIdQuery(trip_id=trip_id)
        return self._get_by_id_handler.handle(query)

    def list_trips_by_owner(
        self, owner_id: str, status: Optional[str] = None
    ) -> List[TripDto]:
        """Возвращает список TripDto для заданного владельца."""
        query = ListTripsByOwnerQuery(owner_id=owner_id, status=status)
